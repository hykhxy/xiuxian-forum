...
const api = {
  ...
  // multipart 文件上传（FormData；浏览器自动设 Content-Type boundary）
  async postForm(p, formData) {
    const token = localStorage.getItem('token');
    let res;
    try {
      res = await fetch(API_BASE_URL + p, {
        method: 'POST',
        headers: token ? { Authorization: 'Bearer ' + token } : {},
        body: formData
      });
    } catch (e) {
      return { ok: false, status: 0, message: '无法连接灵脉服务器' };
    }
    let payload;
    try { payload = await res.json(); } catch (e) { payload = { success: false, message: '响应异常' }; }
    if (!payload.success && res.status === 401 && !/login\.html/.test(location.href)) {
